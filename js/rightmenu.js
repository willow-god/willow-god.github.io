...
function showMessageOnceInFiveMinutes() {
    ...
    if (currentTime - lastMessageTime >= 5 * 60 * 1000) {
        showMessage('按住ctrl再右键点击可以恢复原菜单哦！');
...
rm.copySelect = function(){
    ...
    showMessage('复制啦！请注意版权信息哦！');
}

//代码复制弹窗
document.addEventListener('DOMContentLoaded', function () {
    var copyButtons = document.querySelectorAll('.highlight-tools .copy-button');
...
async function replySelect() {
    ...
    try {
        ...
        // 显示带评论的弹窗
        showPopupWithComments('https://twikoo.qyliu.top', 'comment-section');
    } catch (error) {
        console.error(error);
    }
